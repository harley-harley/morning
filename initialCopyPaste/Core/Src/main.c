...
#include <string.h>
  ...
  while (1) {
    ...
    // if (HAL_I2C_Master_Transmit(&hi2c1, (0x70), buf, 1, 1000) != HAL_OK) {
    //   strcpy((char *)buf, "Error 70\r\n");
    //   HAL_UART_Transmit(&huart2, buf, strlen((char *)buf), HAL_MAX_DELAY);
    // }
    // if (HAL_I2C_Master_Transmit(&hi2c1, TMP103_ADDR, buf, 1, 1000) != HAL_OK)
    // {
    //   strcpy((char *)buf, "Error Tx\r\n");
    //   HAL_UART_Transmit(&huart2, buf, strlen((char *)buf), HAL_MAX_DELAY);
    //   continue;
    ...
    // } else {
    //   ret_status =
    //       HAL_I2C_Master_Receive(&hi2c1, TMP103_ADDR, buf, 1, HAL_MAX_DELAY);
    //   if (ret_status != HAL_OK) {
    //     strcpy((char *)buf, "Error Rx\r\n");
    //   } else {
    //     // ret_val = ((int16_t)buf[0] << 4) | ((int16_t)buf[1] >> 4);
    //     // if (ret_val > 0x7FFF) {
    //     //   ret_val |= 0xF000;
    //     // }
